...
// Ejercicio 4 del taller de Programación Avanzada 2

const pal = ['edificio', 'perla', 'estrella', 'pescado', 'salero', 'biblioteca', 'guitarra', 'dinero', 'música']

pal.forEach(function (as) {
    let ult = as.at(-1).toLowerCase();

    if (ult == "a") {
        console.log(`La palabra ${as} es un sustantivo femenino`);
    } else {
        console.log(`La palabra ${as} es un sustantivo masculino`);
    }
});

// Ejercicio 5 del taller de Programación Avanzada 2

let cRegEx = /^.+[@].+[.](com)$/
let correo = ['jhonyq@hotmail.com']
if (cRegEx.test(correo)) {
    console.log(`${correo} es un correo electrónico`);
} else {
    console.log(`${correo} NO es un correo electrónico`);
}